...
const LoginModal = ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} extraClass="w-[92%] sm:w-auto">
      {/* Modal Header */}
      <div className="w-[405px]  mt-[16px] space-y-[16px]">
        <span className="text-[16px] text-white font-semibold]">
          Log in to your account in order to <br /> participate in the vote
        </span>
        <p className="font-Medium text-[12px] text-gray-600 mb-4">
          Need help with your account?{" "}
          <span className="text-primary cursor-pointer">Get Support</span>
        </p>
        ...
        {/* Email Login */}
        <p className="font-semibold text-[14px] mb-2">Use a one time code</p>
        <div className="flex items-center gap-4 w-full">
          <div className="relative flex items-center input-field !px-2 sm:!px-4 circle grow">
            ...
            />
            <button
              className="absolute top-2 right-2 sm:hidden flex bg-primary font-semibold p-[6px] circle btn-sm"
            ...
            <button
              className="btn_logincard text-[10px] font-semibold text-black text-[12px] !hidden sm:!flex !py-3"
              onClick={() => handleLogin("email", email)}
              disabled={isSending}
            >
              Send code
        ...
        {emailError && (
          <p className="text-red-400 text-xs mt-2 ml-2">{emailError}</p>
        )}

        {/* OR Separator */}
        <div className="text-center text-gray-500 my-3 sm:my-4 text-xs sm:text-base">
          OR
        </div>

        {/* Log in using X */}
        <button
          className="input-field circle w-full text-gray-600 !px-2 sm:!px-4 text-sm sm:text-base"
