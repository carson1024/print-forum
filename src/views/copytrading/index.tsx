...
const CopyTrading = () => {
  ...
  const [activeTab, setActiveTab] = useState<"public" | "trader" | "favo">(
    "public"
  );
  ...
  const updateSubTab = (
    tab: "portfolios" | "traders" | "favorites",
    subTab: string
  ) => {
        ...
        <div className="grid grid-rows-[76px_64px_1fr] flex-col h-screen border-gray-800">
          <div className="flex border-b border-gray-800 flex items-center">
            <div className=" flex text-[14px] font-semibold text-gray-500">
              {activeTab == "public" ? (
                <button
                  className="ml-[18px] repause_btn flex items-center justify-center mainhover flex"
          ...
          <div className="flex border-b border-gray-800 flex items-center">
            <div className=" flex px-[18px]">
              {activeTag == "pnl" ? (
                <button
                  className="mr-[8px] btn_filter font-semibold text-white text-[12px] flex items-center justify-center flex"
                  onClick={() => setActiveTag("pnl")}
                >
                  PnL
                </button>
              ) : (
                <button
                  className="mr-[8px] btn_filter_before font-semibold text-gray-600 text-[12px] flex items-center justify-center flex"
                  onClick={() => setActiveTag("pnl")}
                >
                  PnL
                </button>
            ...
            <div className="ml-auto">
              <div className="flex gap-2 sm:gap-3">
                ...
                <div className="flex">
                  <button
                    className="text-gray-600 w-[28px] y-[28px] sm:w-8 sm:y-8 round_filter hover:text-primary hover:bg-gray-200"
                ...
                <div
                  ref={wrapperRef}
                  className="relative inline-block text-left mr-[18px]"
                >
                  <button
                  ...
                  >
                    <span>{filters}</span>
                    <AiFillCaretDown className="text-primary/30 ml-1" />
                  </button>
                  {isOpen && (
          ...
          <div className="flex border-b border-gray-800 flex overflow-y-auto h-[calc(100vh-202px)]">
            {isLoading || !users.length ? (
              <div className="m-[18px] flex flex-col gap-5 overflow-auto flex-grow loading">
                <>
                  <SkeletonRow opacity={90} />
                  <SkeletonRow opacity={90} />
                  <SkeletonRow opacity={90} />
                  <SkeletonRow opacity={70} />
                  <SkeletonRow opacity={50} />
