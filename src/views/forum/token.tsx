...
const TokenDetail = () => {
  ...
  const [activeTab, setActiveTab] = useState('top callers');

  const navigate = useNavigate();
  const [isLoginFromVoteModalOpen, setIsLoginFromVoteModalOpen] =
    useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const pairAddress = location.pathname
      ...
      setConfirmVote(
        localStorage.getItem(pairAddress + myuser) === "yes"
          ? 1
          : localStorage.getItem(pairAddress + myuser) === "no"
            ? 2
            : 0
      );
      setUserid(myuser);
      ...
      checkCall(pairAddress).then(async (result) => {
        if (!result) {
          ...
          const top10 = result.topHolders.slice(0, 10).reduce(
            (acc, holder) => {
              acc.pct += holder.pct;
          ...
          if (updateError)
            console.error("Error updating supply:", updateError.message);
      ...
      if (!callsError && calls) {
        ...
        const uniqueCallers = Array.from(
          new Map(calls.map((item) => [item.user_id, item])).values()
        );
        setTopCallers(uniqueCallers);
      }

      if (!itemError && item) setSitem(item);
      if (!adminCommentsError && adminComments)
        setAdminDiscussions(adminComments);
      if (!commentsError && comments) {
        setDiscussions(comments);
        if (session?.user) {
          const userCommented = comments.some(
            (com) => com.user_id === session.user.id
          );
  ...
  const handleVotelike = () => {
    if (!isLogin) {
      setIsLoginFromVoteModalOpen(true);
    } else {
      ...
      const insertUser = async () => {
        ...
        if (data.length > 0) {
          ...
          setRatioVote(
            Math.ceil(
              ((v2[0].like_number + 1) * 100) /
                (v2[0].like_number + 1 + v2[0].dislike_number)
            )
          );
          const { error: updateError } = await supabase
            .from("vote")
          ...
          const { error: insertError } = await supabase.from("vote").insert([
            {
              ...
              like_number: v2[0].like_number + 1,
              dislike_number: v2[0].dislike_number,
              ratio: Math.ceil(
                ((v2[0].like_number + 1) * 100) /
                  (v2[0].like_number + 1 + v2[0].dislike_number)
              ),
    ...
    if (!isLogin) {
      ...
      const insertdislikeUser = async () => {
        ...
        if (data.length > 0) {
          ...
          setRatioVote(
            Math.ceil(
              (v1[0].like_number * 100) /
                (v1[0].dislike_number + 1 + v1[0].like_number)
            )
          );
          const { error: updateError } = await supabase
            .from("vote")
          ...
          const { error: insertError } = await supabase.from("vote").insert([
            {
              call_name: paddress,
              user_id: sitem?.user_id,
              like_number: v1[0].like_number,
              dislike_number: v1[0].dislike_number + 1,
              ratio: Math.ceil(
                (v1[0].like_number * 100) /
                  (v1[0].dislike_number + 1 + v1[0].like_number)
              ),
  ...
  const saveComment = async () => {
    ...
    if (error) {
      showToastr("Can't send your comment!", "error");
    } else {
      setComment("");
    }
  };
  ...
  return <ForumLayout>
    <div className={`border-r border-gray-800 overflow-hidden ${isTopLoading || isLoading ? 'loading' : ''}`}>
      <div className="grid lg:grid-rows-[90px_76px_1fr] flex-col h-screen border-gray-800">
        <div className="flex border-b border-gray-800 flex items-center py-2.5 px-4 ">
          <button onClick={() => navigate(-1)} ><img src={Prev} className="w-[24px] h-[24px] mr-[12px] lg:ml-[12px]" /></button>
          {
          ...
          <div className="space-y-[5px] flex-col flex">
            {
              isLoading ? <div className="skeleton w-24 h-4 sm:w-[300px] sm:h-[20px] rounded"></div> :
                <div className="flex space-x-[12px] items-center">
                  <span className="text-[10px] lg:text-[14px] font-semibold items-center text-white">${callReport?.baseToken.symbol}</span>
                  <div className=" hidden text-gray-600 text-[12px] font-Medium lg:flex">
                  ...
                  </div>
                  {
                    sitem?.is_featured ? <div>
                      <span className={`badge-multiplier-${sitem?.featured}X w-[35px] h-[21px]  text-[#59FFCB] text-[10px] lg:text-[12px] font-semibold items-center flex`}></span>
                    </div> :
                      <></>
                  }
                </div>}
            {
              isTopLoading ? <div className="skeleton w-24 h-4 sm:w-[600px] sm:h-[20px] rounded"></div> :
                <div className=" hidden lg:flex space-x-[6px] items-center">
            ...
            <div className=" flex text-gray-600 text-[10px] lg:text-[12px] font-Medium lg:hidden">
              MCAP {formatNumber(sitem?.init_market_cap)}
              <span className="text-[10px] lg:text-sm items-center flex">
                <AiFillCaretRight />
              </span> {formatNumber(sitem?.changedCap)}&nbsp;
              <span className="text-[10px] lg:text-sm items-center flex font-Regular text-white">
                {
                  sitem?.percentage == 100 ? <></> : sitem?.percentage > 100 ?
                    <div className="flex text-green-600">
                      <AiFillCaretUp className="text-green-600" />
          ...
          <div className={` lg:hidden gap-1 ml-auto mr-[18px] justify-end ${isLoading ? 'hidden' : ''}`}>
        ...
        <div className="flex border-b border-gray-800 flex flex-col lg:flex-row lg:items-center gap-[10px] lg:gap-0 py-2.5 px-4 ">
          {
            isTopLoading ? <div className="skeleton w-24 h-4 sm:w-[600px] sm:h-[20px] rounded"></div> :
              <div className=" flex flex-wrap gap-1.5 lg:hidden space-x-[6px] lg:items-center">
                <span className="token_info text-gray-600 text-[10px] font-Medium">Callers<span className="token_border text-white">{callersCount}</span></span>
                <span className="token_info text-gray-600 text-[10px] font-Medium">Top&nbsp;10&nbsp;holders<span className="token_border text-white">{top10HolderInfo.pct.toFixed(2)}%<span className="text-gray-600">${formatNumber(top10HolderInfo.uiAmount * sitem?.changedPrice)}</span></span></span> <br />
                <span className="token_info text-gray-600 text-[10px] font-Medium space-0 ">Top&nbsp;3&nbsp;holders
                  {top3Holders.map((holder, index) => (
                    <span className="token_border text-white ">{holder.pct.toFixed(2)}%<span className="text-gray-600">${formatNumber(holder.uiAmount * sitem?.changedPrice)}</span></span>
                  ))}
                </span>
              </div>}
          <div className="flex gap-2 ">

            <a href={`https://dexscreener.com/solana/${callReport?.pairAddress.toLocaleLowerCase()}`} target="_blank" className=" lg:ml-[18px] items-center flex token_outsite "><img src={Dexscreener} alt="DEX Screener" className="w-[17px] h-[20px]" /><span className="text-[10px] lg:text-[14px] font-semibold items-center text-white">Dex Screener</span></a>
            <a href={`https://photon-sol.tinyastro.io/en/lp/${callReport?.pairAddress.toLowerCase()}`} target="_blank" className=" lg:ml-[12px] items-center flex token_outsite "><img src={Photon} alt="Photon-SOL" className="w-[17px] h-[20px]" /><span className="text-[10px] lg:text-[14px] font-semibold items-center text-white">Photon-SOL</span></a>
            <div onClick={handleCopy} className=" lg:ml-[12px] items-center flex token_outsite "><span className="text-[14px] flex font-semibold items-center text-white">CA</span><span className="text-gray-600 text-[12px] font-Medium">{formatShortAddress(callReport?.pairAddress)}
              <button>{
          ...
          <div className={`hidden lg:flex gap-1 ml-auto mr-[18px] justify-end ${isLoading ? 'hidden' : ''}`}>
            <div className="flex gap-1 items-center">
              {confirmVote == 0 ?
                <div className="flex gap-1 items-center">
                  <button className="circle-reitem w-6 h-6 bg-gray-100 hover:bg-gray-50 text-green-600 text-sm pb-[2px]" ><AiFillCaretUp onClick={handleVotelike} />
        ...
        <div className="flex py-[10px] border-b border-gray-800 flex items-center justify-between px-[16px]">
          <div className="flex lg:hidden gap-2">
            <button
              ...
              onClick={() => setActiveTab('top callers')}
            >
              Top Callers
            </button>
        ...
        <div className=" hidden lg:grid h-screen" style={{ gridTemplateColumns: 'calc((100vw - 501px) / 2) 1fr' }}>
          ...
          <div className="flex border-b border-gray-800 flex items-center ">
            <a
              ...
              target="_blank"
              className="ml-[18px] items-center flex token_outsite "
            >
              <img
                src={Dexscreener}
                alt="DEX Screener"
              ...
              />
              <span className="text-[14px] font-semibold items-center text-white">
                Dex Screener
              </span>
            </a>
            <a
              ...
              target="_blank"
              className="ml-[12px] items-center flex token_outsite "
            >
              <img
                src={Photon}
                alt="Photon-SOL"
            ...
            <div
              ...
              className="ml-[12px] items-center flex token_outsite "
            >
              <span className="text-[14px] flex font-semibold items-center text-white">
                CA
              </span>
              <span className="text-gray-600 text-[12px] font-Medium">
                {formatShortAddress(callReport?.pairAddress)}
                <button>
                  {!isCopied ? (
                    <img
                      src={IconCopy}
                      className="ml-[6px] text-white w-[12px] h-[12px] brightness-50"
                    />
                  ) : (
                    <span className="ml-[6px] text-[#06cf9c] items-center flex">
                      <MdCheck size={16} />
                    </span>
                  )}
                </button>
            ...
            <div
              className={`flex gap-1 ml-auto mr-[18px] justify-end ${isLoading ? "hidden" : ""}`}
            >
              <div className="flex gap-1 items-center">
                {confirmVote == 0 ? (
                  <div className="flex gap-1 items-center">
                    <button className="circle-reitem w-6 h-6 bg-gray-100 hover:bg-gray-50 text-green-600 text-sm pb-[2px]">
                      <AiFillCaretUp onClick={handleVotelike} />
                    </button>
                    <button
                      className="circle-reitem w-6 h-6 bg-gray-100 text-red-400 text-sm pt-[2px]"
                  ...
                  <>
                    {confirmVote == 1 ? (
                      <div className="flex gap-1 items-center">
                        <button className="circle-reitem w-6 h-6 bg-gray-100 hover:bg-gray-50 text-green-600 text-sm pb-[2px]">
                          <FaThumbsUp className="text-green-500 cursor-pointer hover:scale-110 transition-transform" />
                        </button>
                        <button className="circle-reitem w-6 h-6 bg-gray-100 text-red-400 text-sm pt-[2px]">
                          <AiFillCaretDown />
                        </button>
                      ...
                      <div className="flex gap-1 items-center">
                        ...
                        <button className="circle-reitem w-6 h-6 bg-gray-100 text-red-400 text-sm pt-[2px]">
                          <FaThumbsDown className="text-red-500 cursor-pointer hover:scale-110 transition-transform" />
                        </button>
                        <span className="text-xs text-gray-600">
                          {ratioVote}%
                        </span>
        ...
        {activeTab === 'top callers' && (
          <div className="lg:hidden">

            <div className="overflow-y-auto py-2.5 h-[calc(100vh-150px)] px-4">
              {isLoading || !topCallers.length ? (
                <SkeletonList />
              ) : (
                topCallers.map((caller, index) => (
                  <Link to={`/profile?id=${caller.users.id}&tag=1`} key={index}>
                    <div className="mb-[18px]">
                      <div className="topcaller_border items-center flex justify-between">
                        ...
                        <div className="items-center">
                          ...
                          <div className="flex items-center space-x-[6px]">
                            <div className="flex space-x-[12px] items-center">
                              <div className="text-[#76767E] text-[10px] font-Medium flex">
                                MCAP {formatNumber(caller.init_market_cap)}
                                ...
                                <span className="text-sm items-center flex font-Regular text-white">
                                  {caller?.percentage === 100 ? null : caller?.percentage > 100 ? (
                                    <div className="flex text-[#76767E] text-[10px]">
                                      <AiFillCaretUp className="text-[#76767E]" />
                                      <span>{Number(caller?.percentage) - 100}%</span>
                                    </div>
                                  ) : (
                              ...
                              {caller.users?.is_featured && (
                                <span className={`badge-multiplier-${caller.users?.featured}X w-[35px] h-[21px] text-[#59FFCB] text-[10px] font-semibold items-center flex`} />
                              )}
                            </div>
                          </div>
                          <div className="flex items-center space-x-[6px]">
                            <span className={`badge-rank-${caller.users.rank} w-[20px] h-[20px]`}></span>
                            <span className="text-[10px] font-Medium text-white">{caller.users.name}</span>
                            <span className="text-[10px] font-Medium text-[#76767E]">{caller.users.winrate}%</span>
                          </div>
                        </div>
                        </div>
                        <div className=" flex">
                          {caller.addXP > 5 ? (
                            <div className="caller_rise flex items-center mr-[18px]">
                              <span className="text-[12px] font-semibold text-primary">+{caller.addXP}XP</span>
                            </div>
                          ) : caller.addXP !== 0 ? (
                            <div className="caller_rise flex items-center mr-[18px]">
                              <span className="text-[12px] font-semibold text-red-300">{caller.addXP}XP</span>
                            </div>
                          ) : null}
                          <span className="text-[#76767E] text-[12px] font-semibold items-center flex">
                            {formatTimestamp(caller.created_at)}
                          </span>
                        </div>
                      </div>
                    </div>
          ...
          <div className="lg:hidden">
            ...
            <div className="overflow-y-auto py-2.5 h-[calc(100vh-150px)] px-4">
              {!isLoading && preshow && (
                <div>
                  <div className="flex items-center mb-[2px]">
                    <span className={`badge-rank-${me[0].rank} w-[20px] h-[20px] mr-[6px]`} />
                    <span className="text-[14px] font-Medium text-white mr-[6px]">{me[0].name}</span>
                    <span className="text-[12px] font-Medium text-[#76767E] mr-[6px]">{me[0].winrate}%</span>
                    <span className="text-[12px] font-Medium text-[#76767E] ml-auto">{formatTimestamp(me[0].created_at)} ago</span>
                  </div>
                  ...
                  <div className="border-b border-gray-800 mt-[18px] mb-[18px]" />
                </div>
              )}
              {isLoading || !discussions.length ? (
                <SkeletonDiscussionList />
              ) : (
                discussions.map((discussion, index) => (
                  <div key={index}>
                    <div className="flex items-center mb-[2px]">
                      <span className={`badge-rank-${discussion.users.rank} w-[20px] h-[20px] mr-[6px]`} />
                      <span className="text-[14px] font-Medium text-white mr-[6px]">{discussion.users.name}</span>
                      <span className="text-[12px] font-Medium text-[#76767E] mr-[6px]">{discussion.users.winrate}%</span>
                      <span className="text-[12px] font-Medium text-[#76767E] ml-auto">{formatTimestamp(discussion.created_at)} ago</span>
                    </div>
              ...
              {!isLoading && admindiscussions?.map((admin, index) => (
                <div key={`admin-${index}`}>
                  <div className="flex items-center mb-[2px]">
                    <span className="badge-rank-10 w-[20px] h-[20px] mr-[6px]" />
                    <span className="text-[14px] font-Medium text-white mr-[6px]">Administrator</span>
                    <span className="text-[12px] font-Medium text-[#76767E] ml-auto">{formatTimestamp(admin.created_at)} ago</span>
                  </div>
                  <div className="items-center">
                    <span className="text-[12px] font-Medium text-gray-600">{admin.comment}</span>
                  </div>
                  <div className="border-b border-gray-800 mt-[18px] mb-[18px]" />
                </div>
              ))}
            ...
            {!isLoading && isLogin && (
              <div className="mt-auto px-4">
                <div className="relative rounded-full bg-gray-100 px-[12px] circle py-2 flex items-center">
                  <input
                    type="text"
