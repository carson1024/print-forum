...
const ForumList = () => {
  ...
  const [filters, setFilters] = useState(
    searchParams.get("level") || "All Ranks"
  );
    ...
    const fetchCalls = async () => {
      ...
      if (data) {
        if (filters == "All Ranks" || filters == null) {
          ...
          setCallListFeatured(
            data
              .filter((call) => call.is_featured === true)
              .filter(
                (call) => call.users.rank === parseInt(filters.slice(6, 8), 10)
              )
          );
          setCallListLastest(
            data
              .filter((call) => call.is_featured === false)
              .filter(
                (call) => call.users.rank === parseInt(filters.slice(6, 8), 10)
              )
          );
          ...
          <div className="flex border-b border-gray-800 flex items-center justify-between px-[18px]">
            <div className="flex show_filter items-center text-[14px] font-semibold text-gray-500">
              <div ref={wrapperRef} className="relative inline-block text-left">
                <span>Showing</span>
                <button
                  className="text-white whitespace-nowrap text-[14px] font-semibold ml-[8px]"
                ...
                <div className="flex-1 overflow-y-auto h-[calc(100vh-202px)]">
                  ...
                  >
                    {isLoading || !callListFeatured.length ? (
                      ...
                      callListFeatured.map((item) => (
                        <CallRow call={item} key={item.id} />
                      ))
                    )}
                  </div>
                </div>
